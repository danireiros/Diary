<template>
  <div :class="['relative border dark:border-gray-700 rounded-2xl overflow-hidden', isToday? 'ring-2 ring-amber-400':'bg-white dark:bg-gray-800']">
    <div class="grid grid-cols-[60px_1fr]">
      <div class="border-r dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
...
function topOf(d){ return (d.getHours()*60 + d.getMinutes()) * (12/60) }
function styleFor(o){ const top = topOf(o.s); const height = Math.max(18, (o.e - o.s)/60000*(12/60)); const left = o.lane*8; const width = `calc(100% - ${o.lane*8}px)`; const color = props.categories.find(c=>c.id===o.task.category_id)?.color || '#333'; return { top: top+'px', height: height+'px', left: left+'px', width, backgroundColor: color } }
