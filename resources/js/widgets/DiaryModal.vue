<template>
  <div class="fixed inset-0 bg-black/30 flex items-center justify-center p-3" @click.self="$emit('close')">
    <div class="w-full max-w-2xl bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-2xl shadow-xl">
      <div class="flex items-center justify-between p-3 border-b dark:border-gray-700"><div class="font-semibold">{{ initial? 'Editar entrada':'Nueva entrada' }}</div><button class="px-3 py-1 rounded-lg border dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700" @click="$emit('close')">Cerrar</button></div>
      ...
      <div class="px-4 pb-4">
        <div class="mt-3 flex flex-col gap-2"><label class="text-sm">Contenido</label><textarea class="border dark:border-gray-700 rounded-lg px-3 py-2 min-h-[120px] bg-white dark:bg-gray-900" v-model="content" /></div>
