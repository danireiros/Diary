<template>
  <div class="border dark:border-gray-700 rounded-2xl bg-white dark:bg-gray-800 overflow-hidden">
    <div class="grid grid-cols-[60px_repeat(7,1fr)] border-b dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
      <div />
      <div v-for="d in days" :key="d.toISOString()" class="p-2 text-sm font-medium border-l dark:border-gray-700" :class="{ 'bg-amber-50 dark:bg-amber-900/20': isSameDay(d, now) }">
        {{ d.toLocaleDateString(undefined,{weekday:'short', day:'numeric', month:'short'}) }}
      </div>
    </div>
    <div class="grid grid-cols-[60px_repeat(7,1fr)]">
      <div class="border-r dark:border-gray-700">
        <div v-for="h in 24" :key="h" class="h-12 border-b dark:border-gray-700 text-xs text-gray-500 flex items-start justify-end pr-2 pt-1">{{ h-1 }}:00</div>
      </div>
      <div v-for="(d,idx) in days" :key="idx" class="relative border-l dark:border-gray-700" :class="{ 'bg-amber-50/30 dark:bg-amber-900/10': isSameDay(d, now) }">
        <div v-for="h in 24" :key="'g'+h" class="h-12 border-b dark:border-gray-700" />
...
function styleFor(o){ const top = (o.start.getHours()*60+o.start.getMinutes())*(12/60); const height = Math.max(18, (o.end - o.start)/60000*(12/60)); const left = o.lane*6; const width = `calc(100% - ${o.lane*6 + 2}px)`; const color = props.categories.find(c=>c.id===o.task.category_id)?.color || '#333'; return { top: top+'px', height: height+'px', left: left+'px', width, backgroundColor: color } }
